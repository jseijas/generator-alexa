...
module.exports = yeoman.Base.extend({
  ...
  askFacts: function(done) {
    var prompts = [{
      type: 'input',
      name: 'fact',
      message: 'Enter an fact (enter to finish)',
    }];
    if (!done) {
      done = this.async();
    }
    ...
    return this.prompt(prompts).then(function(answer) {
      if (answer.fact && answer.fact !== '') {
        this.props.facts.push(answer.fact);
        this.askFacts(done);
      } else {
        done();
      }
  ...
  writing: function () {
    ...
    for (var i = 0; i < this.props.facts.length; i++) {
      this.props.factsStr += this.props.facts[i]+'\n';
    }
    ...
    this.template(source, target, this.props);
    source = '_fact.txt'
    var target = this.props.intentName+'-intent.txt';
    this.template(source, target, this.props);
  }
});
